...
const deleteSong = (id) => {
    ...
    if (userData?.songs.length === 0) {
        ...
        const resetText = document.createElement("Reset Playlist");
...
const setPlayButtonAccessibleText = () => {
    ...
    playButton.setAttribute('aria-label', song?.title ? `Play ${song.title} by ${song.artist}` : 'Play');
}
...
const highlightCurrentSong = () => {
    ...
    playlistSongs.forEach((songEl) => {songEl.removeAttribute('aria-current')
        if (songToHighlight) {
